...
| [`distribution.md`](docs/distribution.md) | 开源 npm/二进制分发、全新环境、升级卸载和供应链 | 修改安装包、`self --init`、发布或首次运行体验 |
...
| 领域 | 入口 | 负责内容 |
| --- | --- | --- |
| Workspace | [`domains/workspace/`](docs/domains/workspace/) | Self Root、配置、`self --init`、系统组件自检和能力发现 |
...
- [`initialization.md`](docs/domains/workspace/initialization.md)：`self --init` Wizard、System Preflight、来源/模型配置、首次索引、组件/模型自检和 Setup Spec
...
- 修改安装、首次引导或公开发布：读 Distribution、Workspace Initialization、Technology Stack、Engineering Standards 和 Testing。
