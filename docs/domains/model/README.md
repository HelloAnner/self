...
> 状态：Phase 6 Embedding、Graph 抽取与带证据 Ask 的结构化 Chat Provider/Model/Invocation 已实现（Schema 7）
...
- Graph 抽取使用 `chat` capability 和版本化 Prompt/Schema；响应未经精确 Evidence 与领域约束校验不能进入 Graph。
...
- [`model.md`](./model.md)：Provider、Model、Invocation、漂移和凭证边界。
- [`schema.md`](./schema.md)：Schema 5 Model Registry 与审计表。
- [`workflows.md`](./workflows.md)：注册、调用、重试、Circuit Breaker 和 Sentinel。
- [`commands.md`](./commands.md)：Phase 4 Model CLI 契约。
- [`testing.md`](./testing.md)：Fixture Provider 与可选 Live Suite。
