...
> 实现状态：Phase 5 Gate 已通过；证据位于 `.test-runs/roadmap/2026-07-11/phase-5/`。
...
- 相同主体和 Predicate 指向多个普通 object 时不自动冲突。
- 只有双方相同且非空的 conflict_scope、相同时间和限定范围内不能同时为真时，规则才创建 ConflictSet。
- 旧的隐式 ConflictSet 被解决后，相关 Claim 从 disputed 恢复到证据决定的状态和可信度。
- 否定、条件、版本和时间限定不丢失。
...
- Artifact 的 SVG 页面节点可以回到 Entity/Claim/Chunk；未来 Cytoscape 渲染器必须复用同一追溯断言。
...
- 编译后 CLI 覆盖显式 Wiki/Markdown/Embed、missing reference、Entity identity、Claim Evidence、Conflict、七维 Confidence、Plan/Apply、受限遍历、三种导出、shadow Diff/rollback、崩溃门禁和 Schema 5→6。
- 同一 Blob 的两份转载绑定两条 Evidence，但 `independent_source_count` 保持 1；互斥 Claim 同时保留并进入同一 ConflictSet。
- SemanticNeighbor 绑定精确 VectorSpace，Top-K 硬上限为 8；它只存在于派生表，不自动创建事实 Relation。
- 真实 `~/notes` 只读归档形成 19,136 个节点、24,478 条 Active Generation 关系；不修改外部 Vault，公开证据只保存聚合计数。
- DashScope `qwen3.7-plus-2026-05-26` 在两段隔离合成资料上完成真实结构化抽取；不符合 Schema、枚举、Domain/Range 或精确摘录的中间响应均在发布前被拒绝，最终形成 4 个 Entity、2 个 Claim 和 2 条 Evidence。Key 仅由临时环境变量提供。

## 14. Phase 9 已执行证据

- Entity/Relation/Claim confirm/reject 以 `--if-version` 和幂等键提交，保留证据与 Generation，并精确使依赖 Answer、Topic 和 Artifact stale/needs_review。
- Entity/Relation/Claim delete 只创建 Plan；Apply 软删除对象及 GraphNode，Restore 保留相同稳定 ID 并从 OperationChange 恢复 before 状态。
- 已 rejected/deleted/redirected 等终态 GraphNode 不会被 Source Delete 错误降级为 stale；审计表和逐对象变化表由 SQLite Trigger 保证不可变。
