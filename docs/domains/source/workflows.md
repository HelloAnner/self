...
Delete 必须先生成 `automation.mutation/source_delete` Plan，记录 Source version、当前 Snapshot、精确依赖集合及其 before/after。Apply 在所有前置条件未变化时用一个事务软删除 Source/Document、tombstone Chunk、detach Connection，并把有证据依赖的 Graph、EvidenceContext、Answer、Topic 和 Artifact 标记 stale/needs_review。Restore 读取原 OperationChange，恢复同一组稳定 ID 和历史状态，同时递增可版本对象。

Purge 是与 Delete 分离的不可逆流程：Source 必须已经 deleted，且 Connection、Note、IngestionRun、Document、EvidenceContext、Topic Citation 和 Graph 引用计数全部为零。Plan 列出专属 Snapshot Manifest、Blob 和 Hash；Apply 先把 Root 内文件移入 Root-local staging，数据库事务提交后才 unlink，数据库失败时将文件移回。共享 Blob 永不随单个 Source purge 删除。
