...
| RetrievalRun | 一次固化检索计划、索引水位、候选和阶段 Trace 的运行 | Retrieval |
| EvidenceContext | 某次回答实际使用且可重放的最小证据集合 | Retrieval |
| Answer | 基于一个 EvidenceContext 发布的可失效回答 | Retrieval |
| Statement | Answer 内带类型、可信度和支持状态的原子结论 | Retrieval |
| Citation | Statement 到 EvidenceContext Item 精确原文范围的映射 | Retrieval |
...
| OperationChange | Operation 对单个对象的不可变 before/after、版本和执行状态 | Automation |
| AuditEvent | Plan 与 Operation 生命周期中的不可变审计事件 | Automation |
| Job | 可 checkpoint、取消和恢复的长任务 | Automation |
| Backup | SQLite 一致性快照和逐文件 Hash Manifest 组成的可恢复版本 | Operations |
| VerificationRun | 一次持久化的完整性检查及其问题集合 | Operations |
| GcReceipt | 引用证明 GC Apply 后的不可变候选与逐项结果 | Operations |
| MaintenanceLease | Root 内维护操作的跨进程互斥所有权和过期边界 | Operations |
...
| RetrievalRun、RetrievalPlan、EvidenceContext、Answer、Statement、Citation、Trace | Retrieval | Query Result / Retrieval Read Model |
...
- Plan 核心、Target、OperationChange 和 AuditEvent 不可变；状态只允许按领域状态机推进，Restore/Undo 必须产生新 Operation，不能改写历史。
- 同一幂等键只可绑定同一规范输入；相同输入返回首次结果，不同输入必须冲突。
- `atomic` 表示全部逐项结果在一个事务提交；尚未实现部分成功时不得声明 `per_item`。
