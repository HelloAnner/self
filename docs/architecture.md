...
知识图谱直接保存在同一个 SQLite 的邻接表、Entity、Relation、Claim 和 Evidence 表中；邻居查询使用组合索引，有界路径使用 Recursive CTE。语义 Top-K 单独保存为绑定 VectorSpace 的可重建投影，不能冒充事实边。JSON-LD/GraphML 只是导出格式；Phase 8 Artifact 使用有限原生 SVG，未来若引入 Cytoscape.js，它也只是展示层。只有真实 Benchmark 证明超过 SQLite 能力后，才允许增加可删除、可重建的专用图索引。完整设计见 [`domains/graph/`](./domains/graph/)。
...
| Note / Topic / Topic Snapshot | note:note_123、topic:top_123、topic-snapshot:tsp_123 |
| Synthesis / Report Section / Conclusion | synthesis:syn_123、section:sec_123、conclusion:concl_123 |
| Topic Citation / Knowledge Gap | topic-citation:tcit_123、knowledge-gap:gap_123 |
...
实现阶段说明：Phase 10 主体已启用 Source→Ingestion→Document/Revision/Chunk→active FTS/Vector→Schema 6 Graph/Claim→Schema 7 Retrieval/Evidence→Schema 8 TopicSnapshot→Schema 9 Page IR/Artifact Build 的完整链，并以 Schema 10 Plan/OperationChange/AuditEvent 包住安全修改。Graph 或 Ask 模型输出未经 Schema、精确摘录、Predicate/Domain 或 Citation 回映校验不会发布；Topic 的关键结论继续沿 Claim/Chunk 回到原始证据。Artifact 使用 React 静态渲染和 Root-local 资源，支持不可变历史、增量组件复用、Diff、多/单文件离线 HTML 与显式 Export。Schema 11 进一步把 Job/Event、Backup、Verification、GC Receipt 和 Maintenance Lease 纳入同一 SQLite；Workspace Backup/Restore、Deep Verify 和引用证明 GC 已通过本机与真实 notes 演练。公开 v1.0 仍等待 24h Soak 和远端跨平台 Gate。
...
Phase 4 已实现 Embedding Model Registry、无正文 Invocation 审计、FTS shadow Generation、维度 vec0 表 + VectorSpace partition、Query Cache 与 text/vector/hybrid Search。Phase 10 起 `vector-space build --detach|--wait` 由 Schema 11 durable Job 调度并持久化 checkpoint、attempt、progress 和 lease。Hosted 调用在 `models.offline=true` 时稳定拒绝，API Key 只从配置指定的环境变量读取。
